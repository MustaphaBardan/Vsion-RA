...
vector<Mat> slice(Mat &image) {
    vector<Mat> tiles;
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            Mat tile = image(Range(i, min(i + image.rows / 3, image.rows)), Range(j, min(j + image.cols / 3, image.cols)));
...
{
    Mat image = imread("cheeta.jpg");
    vector<Mat> slicedImg = slice(image);
   ...
   arucoManager = new ArUco("camera.yml", 0.105f, slicedImg);
